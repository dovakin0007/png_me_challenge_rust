...
impl Png {
    ...
    pub fn from_chunks(chunks: Vec<Chunk>) -> Png {
        ...
        //creates PNG from chunks
    }


    // adds in new chunk by phusing into vector
    pub fn append_chunk(&mut self, chunk: Chunk){
        self.chunks.push(chunk);
    }


    //removes chunks based on index
    pub fn remove_chunk(&mut self, chunk_type: &str) -> std::result::Result<Chunk, ()>{
    ...
    // returns the header which is a constant
    pub fn header(&self) -> &[u8; 8]{
        &self.header
    }

    // return a ref to Chunk array
    pub fn chunks(&self) ->&[Chunk]{
        &self.chunks
    }

    // returns Chunk from vector if chunk_type exists
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {

        match self.chunks.iter().find(|chunk| chunk.chunk_type().to_string()  == chunk_type) {
    ...
    pub fn data_string_by_type(&self, chunk_type: &str) -> Option<String>{
        ...
        if let Some(data) = chunk_data {
            let chunk_data = data.data_as_string().unwrap();
    ...
    //return bytes of chunk as Vector
    pub fn as_bytes(&self)-> Vec<u8>{
        self.header.iter().chain(
            self.chunks.iter().flat_map(|chunk| chunk.as_bytes()).collect::<Vec<u8>>().iter()
...
impl TryFrom<&[u8]> for Png{
    type Error = Error;
    //takes in a value as [u8] and return Png struct
     fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let (header, chunk_bytes) = value.split_at(8);
        //spliting the header and chunk_bytes
        ...
        loop {
            let length = u32::from_be_bytes(chunk_bytes[pos..pos + 4].try_into().unwrap());
            //12 will be the offset of the chunk and will start from the position 0 and increment
            chunks.push(Chunk::try_from(&chunk_bytes[pos..pos + length as usize + 12]).map_err(|_| Box::new(E))?);
            //if goes out of bound returns error
            pos = pos + length as usize + 12;

            // if position is greater return error
            if pos > chunk_bytes.len(){
                return Err(Box::new(E));
            }
            if chunk_bytes.len() == pos {
                break;
            }
